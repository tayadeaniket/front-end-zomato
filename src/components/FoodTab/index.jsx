...
const MobileTabs = () => {
    ...
    return (
        <>
            <div className='lg:hidden bg-white shadow-lg fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500'>
                {allTypes.map((item) => (
                    <Link key={item.id} to={`/${item.id}`} className='w-1/4'>
                        <div className={classnames(
                            "flex text-center flex-col  items-center text-xl",
                            {
                                "relative text-zomato-400" : type === item.id,
                            }    
                        )}
                        >
                            <div className={classnames(
                            ...
                            {
                                "w-full h-full border-t-2 p-3 border-zomato-400" : type === item.id,
                            })
                            }>
                                {item.icon}
                    ...
                    <Link key={item.id} to={`/${item.id}`} className='w-38'>
                        <div
                            className={classnames(
                                "flex items-center gap-3 pd-2 transition duration-700 ease-in-out",
                                { "border-b-2 border-zomato-400": type === item.id }
                            )}>
                            <div
                                className={classnames(
                                    "w-16 h-16 bg-gray-200 p-4 rounded-full",
                                    {
                            ...
                            <h3
                                className={
                                    type === item.id
                                        ? "text-md font-semibold text-zomato-400"
